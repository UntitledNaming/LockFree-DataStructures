...
- 해결 방식: next를 0xFFFF…로 밀어 삽입 진행 상태 보호(개선판은 nullptr에 태그), Qid+CAS128로 소유 검증(개선판은 전역 태그+CAS64), 자체 풀로 노드 보존.
...
- 목적: 큐 삽입 중 노드를 다른 스레드가 오인해 발생하는 순서 꼬임을 재현하고, next를 0xFFFF…로 미는 방식으로 해결한다(개선판은 nullptr 태그 방식).
...
- 개선판(`include/LFQSingleTag.h`): next를 nullptr로 만들 때 상위 17비트에 전용 발급 카운터(m_NextCnt)의 태그를 실어 기록하고, 1st CAS는 내가 본 next에서 주소부를 지운 값(태그 붙은 nullptr, `& TAGMASK`)을 비교값으로 사용한다. 재사용된 노드의 새 nullptr은 태그가 달라 CAS가 실패한다.
...
- 개선판(`include/LFQMultiTag.h`): next 태그 방식을 공용 풀에 적용하되 발급 카운터를 `static UINT64 m_NextCnt`로 두어 모든 큐가 하나의 발급원을 공유한다. 태그가 큐를 가로질러 유일하므로 Qid 없이 CAS64만으로 재사용 오인 삽입이 차단된다.
...
- 관련 테스트: 스레드 1/3/6/36 비교(`docs/results/`, 3분 재측정).
- 확인 필요: `test/`의 Lock-Free 스택은 계측 인자를 받는 버전으로 `include/`판과 시그니처가 다르다. 실측 수치는 `docs/Test_Report.md` §3에 확정 정리됨.

> 각 흐름의 설계 의도는 `docs/Code_Intent.md`, 측정은 `docs/Test_Report.md`를 참고하세요. 재현/해결 영상 링크는 `docs/videos/`에 참고용으로 있습니다.
